...
const Navbar = ({ onLogoutClick }: { onLogoutClick?: () => void }) => {
  ...
  useEffect(() => {
    function checkAuth() {
      setHasAuth(!!localStorage.getItem("authToken"));
    }
    checkAuth();
    ...
    window.addEventListener("authTokenChanged", handleCustom);
    return () => {
      window.removeEventListener("storage", handleStorage);
      window.removeEventListener("authTokenChanged", handleCustom);
    };
  }, []);
          ...
          {NAV_LINKS.map((link) =>
            (!hasAuth && link.label === "Sign Up") ? (
              <div
                ...
                onMouseLeave={() => {
                  signupTimeout.current = setTimeout(() => setSignupDropdown(false), 120);
                }}
              >
                <button
              ...
              <div
                ...
                onMouseLeave={() => {
                  loginTimeout.current = setTimeout(() => setLoginDropdown(false), 120);
                }}
              >
                <button
                  className="text-base font-semibold text-blue-700 hover:text-cyan-500 transition-colors font-sans px-2 py-1 rounded focus:outline-none focus:ring-2 focus:ring-cyan-300"
              ...
              link.label !== "Sign Up" && link.label !== "Login" && (
                <Link
                  key={link.label}
                  href={link.href}
                  className="text-base font-semibold text-blue-700 hover:text-cyan-500 transition-colors font-sans px-2 py-1 rounded focus:outline-none focus:ring-2 focus:ring-cyan-300"
